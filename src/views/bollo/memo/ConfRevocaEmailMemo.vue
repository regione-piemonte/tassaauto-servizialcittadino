...
import { MEMO_REVOCA_EMAIL } from '@/store/actions.type'
...
export default {
  ...
  async created () {
    ...
    store
      .dispatch(MEMO_REVOCA_EMAIL, { email: this.$route.query.email, token: this.$route.query.token })
      .then(() => {
        this.detailSuccess = {
          title: 'Disattivazione Memobollo via e-mail',
          message: 'Memobollo disattivato correttamente!'
        }
      })
      .catch((error) => {
