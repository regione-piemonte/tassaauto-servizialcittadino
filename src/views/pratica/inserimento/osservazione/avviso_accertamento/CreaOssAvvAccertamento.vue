...
import OssAvvAccFormRadioP from '@/components/pratica/osservazione/avviso_accertamento/OssAvvAccFormRadioP'
import OssAvvAccFormRadioNP from '@/components/pratica/osservazione/avviso_accertamento/OssAvvAccFormRadioNP'
import OssAvvAccFormRadioAltro from '@/components/pratica/osservazione/avviso_accertamento/OssAvvAccFormRadioAltro'
...
export default {
  ...
  computed: {
    ...
    intestAvvAcc: function () {
      if (this.avvisoAccertamento.intestatario.denominazione !== null && this.avvisoAccertamento.intestatario.denominazione !== '') {
        // persona giuridica
        return this.avvisoAccertamento.intestatario.denominazione
      } else {
        return this.avvisoAccertamento.intestatario.nome + ' ' + this.avvisoAccertamento.intestatario.cognome
      }
    },
    allegatiErrors () {
      const errors = []
      ...
      this.ossAvvAccertamentoAllegati.length === this.numMaxAllegati && errors.push('Non puoi allegare altri documenti.')
      return errors
    }
  },
  methods: {
    ...
    aggiungiMotivazione () {
      switch (this.activeTab) {
        case 0:
          ...
          this.$refs.formRadioPagato.$v.grigliaAccertamentoPagato.$touch()
          if (this.$refs.formRadioPagato.$v.grigliaAccertamentoPagato.$invalid) return
        ...
        case 1:
          ...
          this.$refs.formRadioNonPagato.$v.grigliaAccertamentoNonPagato.$touch()
          if (this.$refs.formRadioNonPagato.$v.grigliaAccertamentoNonPagato.$invalid) return
        ...
        case 2:
          ...
          this.$refs.formRadioAltroMotivo.$v.grigliaAccertamentoAltro.$touch()
          if (this.$refs.formRadioAltroMotivo.$v.grigliaAccertamentoAltro.$invalid) return
          this.$refs.formRadioAltroMotivo.updMotivOssAltroMotivo()
          break
      }

      const note = this.$refs.note.getTestoNote()
      if (note !== '') store.dispatch(OSS_AVV_ACC_SALVA_NOTE, note)

      this.$router.push({ name: 'riepilogo_osservazione_accertamento' })
    },
    ...
    caricaAllegato () {
      ...
      formData.append('targa', this.avvisoAccertamento.targaVeicolo)
      formData.append('cognome', this.avvisoAccertamento.intestatario.cognome)

      if (!NavigatorService.checkInternetConnection()) return

      this.overlay = true
      store
        .dispatch(OSS_AVV_ACCERTAMENTO_CARICA_ALLEGATO, formData)
        .then((response) => {
          this.overlay = false
    ...
    scaricaAllegato (idAllegato) {
      this.downloadFileError = null
      const attach = this.ossAvvAccertamentoAllegati.find(p => (p.identificativoArchivio === idAllegato))
    ...
    eliminaAllegato (idAllegato) {
      ...
      this.overlay = true
      store
        .dispatch(OSS_AVV_ACCERTAMENTO_ELIMINA_ALLEGATO, {
          slug: idAllegato,
  ...
  async created () {
    if (OsservazioneService.motivoOssAvvAccNonPagato(this.grigliaAccertamentoNonPagato) !== '') {
      this.activeTab = 1
      return this.activeTab
    }
    if (OsservazioneService.motivoOssAvvAccAltroMotivo(this.grigliaAccertamentoAltro) !== '') {
      this.activeTab = 2
      return this.activeTab
    }
    this.activeTab = 0
